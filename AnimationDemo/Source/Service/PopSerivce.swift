...
extension PopSerivce {
    ...
    static func getAnchorPoint(_ direction: FXTutorialDotTipPop.DotDirection) -> CGPoint {
        var point = CGPoint(x: 0.5, y: 0.5)
        switch direction {
        case .up(dotCenter: let dotCenter, controlSize: _):
            point = CGPoint(x: dotCenter.x / UIScreen.main.bounds.width, y: 0)
        case .down(dotCenter: let dotCenter,  controlSize: _):
            point = CGPoint(x: dotCenter.x / UIScreen.main.bounds.width, y: 1)
        case .none:
            break
        }
        return point
